...
import { originSuffix, activeUserList } from "../whiteboard_tab/tools/SharedTypes";
        ...
        <div className="tct_sidemenu">
            ...
            <IconContext.Provider value={{ className: "menu_icon" }}>
                <nav className="menu_area">
                    <ul>
                        <li className="menu">
                            <NavLink to={`/TctModel/0/${TcTnum}`} activeClassName="tct-menu-active">
                                <BiUserPin />
                                Model
                            </NavLink>
                        </li>
                        <li className="menu">
                            <NavLink to={`/TctPhotographer/0/${TcTnum}`} activeClassName="tct-menu-active">
                                <BiCamera />
                                Photographer
                            </NavLink>
                        </li>
                        <li className="menu">
                            <NavLink to={`/whiteboard/${TcTnum}`} activeClassName="tct-menu-active">
                                <BiNews />
                                Whiteboard
                            </NavLink>
                        </li>
                        <li className="menu">
                            <NavLink to={`/TctWorkflow/${TcTnum}`} activeClassName="tct-menu-active">
                                <BiSitemap />
                                Workflow
                            </NavLink>
                        </li>
...
function AddUserMenu(props) {
    ...
    return (
        <Dialog onClose={handleClose} aria-labelledby="simple-dialog-title" open={open}>
            <DialogTitle id="simple-dialog-title" className={classes.inviteHighlight}>
                Invite User
            </DialogTitle>
            <DialogContent className={classes.paperWidth}>
                <DialogContentText>
                    To invite to this collaboration page, please enter user email address here. We will send invite
                    message to the user.
                </DialogContentText>
                <TextField
                    autoFocus
                ...
                <DialogActions>
                    ...
                    <Button onClick={handleSubmit} color="primary" className={classes.inviteHighlight}>
                        Invite
                    </Button>
                </DialogActions>
            </DialogContent>
...
function Header({ TcTnum, title }) {
    ...
    activeUserList.observe(ymapEvent => {
        ymapEvent.changes.keys.forEach((change, key) => {
            if (change.action === "add") {
                ...
                const socketId = activeUsers.map(e => e.socketId);
                if (socketId.includes(key)) {
                    setActiveUsers(activeUsers.filter(user => user.socketId !== key));
                }
            }
        });
    });
    ...
    // for post
    const [titleInputs, setTitleInputs] = useState(title);

    useEffect(() => {
        if (titleInputs !== title) {
...
export default function TctComponant({ children, linkType }) {
    ...
    const [title, setTitle] = useFetch("/api/tct/fetch/title", param);

    return (
        <div className="whole_wrapper">
            <SideMenu TcTnum={TcTnum} />
            <div className="tct_wrapper" style={linkType ? { overflow: "none" } : { overflow: "hidden" }}>
                <Header TcTnum={TcTnum} title={title.title} />
                {children}
            </div>
        </div>
    );

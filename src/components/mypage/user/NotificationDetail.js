...
function NotificationDetail({ isLogin, props }) {
    ...
    const [notification, setNotification] = useFetch(
    ...
    return (
        <UserMyPage user="user" header={msgTitle} isLogin={isLogin}>
            <div className="mail_upper">
                ...
                <span>받는 사람: 나</span>
                <span>
                    보낸 시간: {new Date(Number(sendTime)).toLocaleDateString()}
                </span>
            </div>
...
function MsgContent({ type, TcTnum }) {
    ...
    if (TcTnum) {
        if (type === "A") {
            return (
                <p>
                    If you want to move to the project page, Click on the link
                    below.
                    <br />{" "}
                    <Link to={`/whiteboard/${TcTnum}`}>
                        "http://localhost:3000/whiteboard/{TcTnum}"
                    </Link>
                </p>
            );
        } else if (type === "B") {
          ...
          return (
            <p>
              Project is denied by manager. Try Again.
            </p>
          );
      }
    } else {
